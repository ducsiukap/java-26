# Explore Java Programming Language

> This repository includes the basics of Java Programming Language.

![Java](https://img.shields.io/badge/java-%23ED8B00.svg?style=for-the-badge&logo=openjdk&logoColor=white)

## Index

**1. [Installation JDK](/JAVA001.installations.md)  
2. [Java basic concepts](/JAVA002_basics/README.md)  
3. [Regex](/java003_regex.java)  
4. [OOPs](/java004_oops/README.md)  
5. [Interfaces](/java005_interfaces/README.md)  
6. [Exceptions](/java006_exceptions/)  
7. [Collections](/java007_collection/README.md)**

---

## Goals

The main purposes of this repository are:

- To master **Java Core** concepts and syntax.
- To understand **Object-Oriented Programming (OOP)** deeply.
- To build practical projects (Console Apps, Web Apps, **Mobile Apps**, etc.).

---

## Tech stack

Language: Java (JDK-25).  
IDE: Visual Studio Code.

---

## Contact

**[LinkedIn](https://www.linkedin.com/in/duc-pham-b19b66351/)  
[Mail](mailto:ducpv.contact@gmail.com)  
[Instagram](https://www.instagram.com/vduczz/)**
